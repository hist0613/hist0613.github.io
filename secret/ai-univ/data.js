...
const pblData = {
  1: {
    ...
    companies: ["메타에듀시스", "크래비스"],
    objective: "텍스트·이미지·문서 등 다양한 모달리티를 통합 처리하는 AI 시스템을 설계·구현하고, 멀티모달 AI 모델 개발의 실무 역량을 체득한다.",
    content: "문서 파싱 API와 LLM을 활용하여 문서 이해와 시각-언어 통합 추론 과제를 수행한다. 팀별로 멀티모달 RAG 구축, 이미지-텍스트 크로스모달 검색 등의 프로젝트를 8주간 진행하며, 메타버스 교육 플랫폼 및 산업용 머신비전 시스템과 연계한 멀티모달 AI 서비스 프로토타이핑도 포함한다."
  },
  2: {
  ...
  3: {
    ...
    content: "경량화 프레임워크로 대규모 AI 모델의 양자화·프루닝·지식증류를 수행하고 엣지 디바이스 추론 성능을 벤치마크한다. AI 드론 플랫폼 위에서 실시간 객체 인식·추적 과제를 수행하며, 온보드 컴퓨팅 환경에서의 모델 최적화를 실습한다."
  },
  4: {
    ...
    content: "클라우드 인프라에서 트래픽 이상 탐지·예측·QoS 최적화 AI 모델을 구축하고, 클라우드 네트워크 자동 스케일링에 AI를 적용한다. 사이버보안 분석 도구를 활용하여 NIDS, 악성 트래픽 분류, DDoS 탐지 등의 보안 AI 과제를 수행한다."
  },
  5: {
    ...
    content: "대규모 언어모델의 다국어 기능으로 소수언어 번역·요약·QA 시스템을 개발하고, 팀별 다국어 RAG 시스템 구축 및 문화 맥락 반영 챗봇을 구현한다. 다국어 TTS·STT 기술을 연동하여 음성 기반 다국어 인터페이스 프로토타입을 구현하며, 각 팀은 최소 2개 언어 이상을 다루는 서비스를 구현한다."
  },
  6: {
    ...
    content: "DSP 기반 음성 신호처리 기술과 STT/TTS 엔진을 결합하여 다국어 영상 더빙·자막 생성·AI 아나운서 서비스를 팀별로 개발한다. 음성 인터페이스는 embodied AI의 핵심 기술로 주목받고 있어 이 과제는 피지컬 AI 트랙(T7)과의 연계도 자연스럽게 모색한다. 기업 멘토가 음향 신호 전처리·DSP 파이프라인과 다국어 TTS 커스터마이징·로봇AI 응용 관점을 각각 지도한다."
  },
  7: {
    courseName: "피지컬 AI 융합 시스템 개발",
    companies: ["오토기기", "라이드플럭스"],
    objective: "센서 데이터 수집부터 로봇·자율주행 제어까지 피지컬 AI 전체 스택을 팀 단위로 구현하고, 기업 현장의 로봇·모빌리티 과제를 해결함으로써 물리 세계와 AI를 연결하는 실무 역량을 확보한다.",
    content: "자동차 AI 시스템 기술을 기반으로 물류·서비스 로봇의 자율주행·물체 인식·매니퓰레이션 과제를 수행한다. 레벨4 자율주행 풀스택 SW를 시뮬레이션 환경에서 실습하며 SLAM·End-to-End AI 기반 제어를 통합 구현한다."
  },
  8: {
    courseName: "대규모 데이터 분석 및 AI 파이프라인 구축",
    companies: ["브랜치앤바운드", "르하임"],
    objective: "클라우드 기반 빅데이터 인프라 위에서 대용량 데이터의 수집·정제·분석·시각화 전 과정을 수행하고, 기업 실데이터를 활용한 AI 분석 파이프라인 구축 역량을 체득한다.",
    content: "코딩 역량 평가 플랫폼 데이터를 활용하여 학습자 역량 분석·개인화 추천 모델을 구축한다. 스마트 키친 IoT 운영 데이터로 급식 시설 운영 최적화 파이프라인을 설계하며, 도메인 의사결정 지원을 위한 실시간 대시보드 구현까지 전 과정을 수행한다."
  },
  9: {
    ...
    content: "딥러닝 기반 시계열 예측, 금융 특화 LLM을 기반으로 신용평가 모델 구축, LSTM/Transformer 기반 주가 예측, 금융 RAG 챗봇 등을 수행한다. 금융 AI AutoML 솔루션을 활용하여 금융 예측 모델링 자동화 파이프라인을 구축하고 실무 AutoML 워크플로우를 체험한다."
  },
  10: {
    ...
    content: "문서 파싱 API와 LLM을 활용하여 금융 보고서·공시 자동 분석 시스템을 구축하고, LLM 기반으로 시장 뉴스에서 투자 시그널을 추출하는 NLP 파이프라인을 설계한다. 빅데이터 플랫폼 위에서 강화학습 기반 자동 매매 에이전트 또는 팩터 기반 포트폴리오 최적화 모델을 구현한다."
  },
  11: {
    ...
    content: "AI 기반 안전관리 시스템 사례를 바탕으로 AI 윤리 영향평가를 수행하고, 소버린 AI 정책(데이터 주권·AI 안전성)에 대한 분석 리포트를 작성한다. EU AI Act, 한국 AI 기본법 등 글로벌 규제를 비교 분석하여 특정 산업 분야에 대한 AI 규제 적용 가이드라인을 팀별로 설계한다."
  },
  12: {
    courseName: "AI 기반 사회데이터 분석 및 공공서비스 혁신 프로젝트",
    companies: ["유컴패니온", "김박사넷"],
    objective: "공공 데이터와 사회 데이터에 AI·데이터사이언스 기법을 적용하여 사회 현상을 분석하고, 데이터 기반 정책 제안 및 공공서비스 프로토타입을 개발하는 역량을 함양한다.",
    content: "공공데이터포털 개방 데이터와 소셜미디어 데이터를 활용해 여론 분석·인구 이동·민원 텍스트 마이닝 등 사회 현상 분석을 수행한다. AI 광고 에이전트(미디어 믹스 모델링) 기술과 학술 커뮤니티 데이터를 활용하여 비즈니스 인사이트 도출 및 사회 네트워크 분석 프로젝트를 수행한다."
  }
};
...
const companyProfiles = {
  ...
  "노빅":                    "DSP 기반 음성·신호처리 시스템 전문기업",
  "마음AI":                  "TTS·STT 음성AI 기업 — WoRV 조직 주도로 로봇AI·피지컬AI에 본격 피벗 중",
  "오토기기":                "자동차 AI 시스템 전문기업",
  "라이드플럭스":             "레벨4 자율주행 풀스택 SW 전문기업",
  "브랜치앤바운드":           "코딩 역량 평가 플랫폼 '코드트리' 운영 — SW 역량 평가 자동화",
  "르하임":                  "스마트 키친 IoT 시스템 전문기업 — 급식 시설 AI 운영 최적화",
  "에스비코퍼레이션":         "HR 아웃소싱·BPO 전문기업 — AI 기반 안전관리 시스템 자체 개발",
  "유컴패니온":               "빅데이터 분석·AI 광고 에이전트(미디어 믹스 모델링) 전문기업",
  "김박사넷":                "대학원 지도교수 정보·평가 플랫폼 — 학술 커뮤니티 데이터 분석"
};
...
const facultyData = [
  ...
  { name: "손경락", dept: "ICE", position: "교수", major: "전기및전자공학",   keywords: ["무선통신", "정보/부호이론"] },
  ...
  // ── 컴퓨터공학부 (CSE) ────────────────────────────────────────────
  { name: "김승욱", dept: "CSE", position: "교수", major: "소프트웨어학",      keywords: ["컴퓨터그래픽스"] },
  { name: "박천준", dept: "CSE", position: "교수", major: "전기전자공학",      keywords: ["AI 경량화"] },
  ...
  { name: "장익범", dept: "CSE", position: "교수", major: "전기및컴퓨터공학",  keywords: ["딥러닝", "의료영상분석", "생성형AI"] },
  { name: "지수연", dept: "CSE", position: "교수", major: "전기정보공학",      keywords: ["의료인공지능"] },
  { name: "전병환", dept: "CSE", position: "교수", major: "의과학",            keywords: ["딥러닝", "의료 AI", "의료영상 분석"] },
  ...
  { name: "이준현", dept: "CSE", position: "교수", major: "컴퓨터과학",        keywords: ["그래프 AI", "딥러닝"] },
  { name: "곽호진", dept: "CSE", position: "교수", major: "",                  keywords: ["영상처리", "음향처리", "비전공자 SW교육"] },
  ...
  // ── AI 데이터융합학부 (AID) ────────────────────────────────────────
  { name: "장혜진", dept: "AID", position: "교수", major: "산업시스템공학", keywords: ["데이터마이닝"] },
  { name: "최재걸", dept: "AID", position: "교수", major: "지능정보융합",   keywords: ["검색엔진 AI", "자연어 검색"] },
  { name: "장원제", dept: "AID", position: "교수", major: "전기전자공학",   keywords: ["자율주행시스템", "AI"] },
  { name: "박성민", dept: "AID", position: "교수", major: "인공지능학",     keywords: ["추천시스템", "AI"] },

  // ── Finance & AI (FAI) ─────────────────────────────────────────────
  { name: "공형우", dept: "FAI", position: "교수", major: "산업및시스템공학", keywords: ["Finance 데이터 AI 응용"] },
  { name: "안시현", dept: "FAI", position: "교수", major: "산업공학",         keywords: ["AI기반 부동산 가격추정", "핀테크"] },
  { name: "조소윤", dept: "FAI", position: "교수", major: "수학",             keywords: ["XAI", "생성 모형 금융 활용"] },

  // ── Social Science & AI (SSAI) ─────────────────────────────────────
  { name: "홍승혁", dept: "SSAI", position: "교수", major: "바이오엔지니어링", keywords: ["Physical AI", "거대언어모델(LLM)"] },
  { name: "이동현", dept: "SSAI", position: "교수", major: "기술경영",          keywords: ["AI 기술정책", "환경 빅데이터", "기술경영"] },
  { name: "안서영", dept: "SSAI", position: "교수", major: "심리학",            keywords: ["뇌인지 기반 AI"], deptNote: "SSAI·미디어 겸임" },
  { name: "차민철", dept: "SSAI", position: "교수", major: "산업공학",          keywords: ["인간-AI 상호작용"], deptNote: "SSAI·미디어 겸임" },
];
